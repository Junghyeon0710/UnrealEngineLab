...
1. [SRP - 단일 책임 원칙](#srp)
2. [OCP - 개방 폐쇄 원칙](#ocp)
3. [LSP - 리스코프 치환 원칙](#lsp)
4. [ISP - 인터페이스 분리 원칙](#isp)
5. [DIP - 의존성 역전 원칙](#dip)
6. [코드에서 이런 문제가 보이면](#design-smells)
7. [Unreal과 연결해서 보면](#unreal)
8. [정리](#summary)
9. [내가 중요하게 생각하는 SOLID 원칙](#my-perspective)
...
## 9. 내가 중요하게 생각하는 SOLID 원칙

SOLID 원칙 중에서는 개인적으로 **단일 책임 원칙(SRP)**을 가장 중요하게 생각한다.

여러 코드를 작성하고 협업이나 스터디를 하면서 가장 많이 체감한 부분이기도 하다. 프로젝트를 보다 보면 하나의 상위 클래스나 Manager가 너무 많은 역할을 담당하는 경우가 생각보다 자주 있다.

처음에는 공통 기능을 한곳에서 관리할 수 있어서 편해 보이지만, 기능이 계속 추가되다 보면 UI, 데이터 처리, 네트워크, 상태 관리처럼 서로 다른 책임까지 하나의 클래스에 들어가기 시작한다.

이런 클래스는 코드를 읽을 때 정확히 어떤 역할을 하는지 파악하기 어렵고, 수정할 때도 어디까지 영향을 주는지 확인하기 힘들어진다.

개인적으로 유지보수가 어려운 코드에서 가장 자주 보이는 문제 중 하나가 이런 **책임이 너무 많이 모여 있는 클래스**라고 느꼈다.

그래서 새로운 기능을 기존 클래스에 추가할 때는 한 번쯤

> 이 기능이 정말 이 클래스가 가져야 할 책임인가?

를 생각해보는 게 중요하다고 본다.

예를 들어 `Character` 하나가 이동, 인벤토리, 체력, 퀘스트, UI, 네트워크 처리까지 모두 직접 담당하고 있다면 처음에는 접근하기 편할 수 있지만, 시간이 지날수록 수정 영향 범위가 커지고 클래스 자체를 이해하기 어려워진다.

이럴 때는 기능에 따라 `HealthComponent`, `InventoryComponent`처럼 책임을 분리하거나 별도의 Manager나 Service로 나누는 편이 유지보수하기 좋다고 생각한다.

---

두 번째로 중요하게 생각하는 것은 **리스코프 치환 원칙(LSP)**이다.

언리얼에서는 `Actor`, `Widget`, 여러 Base 클래스를 상속해서 사용하는 경우가 많기 때문에 상속 구조를 자주 접하게 된다.

상속 자체는 공통 기능을 관리하기 편하지만, 관계를 억지로 만들거나 상속 단계가 깊어지기 시작하면 부모와 자식의 역할이 점점 모호해지는 경우가 있다.

리스코프 치환 원칙은 어렵게 생각하기보다

> 부모 타입으로 사용할 수 있다고 했으면 자식도 부모가 보장한 행동을 지켜야 한다.

정도로 이해하고 있다.

예를 들어 `AGun`이라는 부모 클래스가 있고, `Fire()`를 호출하면 총을 발사한다고 약속했다고 해보자. 여기서는 탄약이나 재사용 대기시간 등 발사 조건이 충족됐다고 가정한다. 부모의 계약에서 허용한 정상적인 발사 실패 자체가 LSP 위반이라는 뜻은 아니다.
...
이를 사용하는 쪽에서는 구체적인 총 종류를 모르더라도 다음처럼 사용할 수 있다.

```cpp
void UseGun(AGun* Gun)
{
    Gun->Fire();
}
```

권총이나 소총이 자식으로 있다면 구현 방식이 달라도 문제는 없다.

```cpp
class APistol : public AGun
{
public:
...
둘 다 구현 방식은 다르지만 결국 `Fire()`를 호출하면 **총을 발사한다는 부모의 의미**는 지키고 있다.

반대로 이런 자식이 있다고 해보자.

```cpp
class ADisplayGun : public AGun
{
public:
    virtual void Fire() override
    {
...
사용하는 쪽에서는 `AGun`이기 때문에 당연히 발사할 수 있다고 생각하는데, 특정 자식에서는 아무 동작도 하지 않거나 호출 자체가 불가능해진다.
...
라고 했을 때 부모 타입을 사용하던 쪽의 기대가 깨진다.

이런 상황이 생긴다면 단순히 `Fire()` 구현의 문제가 아니라

> 이 클래스가 정말 `AGun`을 상속하는 것이 자연스러운가?

를 다시 생각해볼 필요가 있다고 본다.

특히 상속 구조가 커지면서 특정 자식마다

```text
이 타입에서는 Fire 사용 안 함
이 타입만 따로 처리해야 함
이 자식에서는 Fire 호출하면 안 됨
```

같은 예외가 계속 생기기 시작하면 부모와 자식의 관계 자체가 잘못된 것은 아닌지 확인할 필요가 있다.

이럴 때는 상속 계층을 다시 나누거나, 특정 기능을 Component나 Interface로 분리해서 필요한 객체에만 붙이는 방식이 더 자연스러울 수 있다.

언리얼에서는 기능을 부모에 계속 올리는 것보다 `ActorComponent`로 분리하는 방식이 이런 문제를 해결하는 데 잘 맞는 경우가 많다고 생각한다.

---

정리하면 개인적으로 SOLID에서 가장 많이 보는 부분은 두 가지다.
...
LSP
→ 부모 타입으로 사용할 수 있다고 했으면
   자식도 부모가 보장한 행동을 깨지 않아야 한다.
```

특히 언리얼에서는 상속과 Base 클래스 사용이 많기 때문에, 단순히 공통 코드가 있다는 이유만으로 부모에 기능을 계속 추가하기보다는 **정말 상속 관계가 자연스러운지, 아니면 Component나 별도의 기능으로 분리하는 편이 더 나은지 계속 판단하는 것이 중요하다**고 생각한다.
